...
public sealed class UplinkPreferenceTests
{
    ...
    public async Task TestPenUplinkCodeGeneration()
    {
        ...
        await server.WaitPost(() => uplinkSys.TryAddUplink(_player, 20, PenPreference, null, out _, out _));
        await _pair.RunTicksSync(5);
    ...
    public async Task TestAllUplinkPreferences()
    {
        ...
        foreach (var pref in protoMan.EnumeratePrototypes<UplinkPreferencePrototype>())
        {
            ...
            await server.WaitAssertion(() =>
            {
                var success = uplinkSys.TryAddUplink(_player, 20, pref.ID, null, out var uplinkTarget, out var setupEvent);
    ...
    public async Task TestImplantUplinkBalance()
    {
        ...
        await server.WaitAssertion(() =>
        {
            var success = uplinkSys.TryAddUplink(_player, startingBalance, implantPreference, null, out var uplinkTarget, out _);
...
public sealed class UplinkFallbackTests
{
    ...
    public async Task TestAddUplinkFallbackToImplant()
    {
        ...
        await server.WaitAssertion(() =>
        {
            ...
            var success = uplinkSys.TryAddUplink(dummy, 20, PenPreference, null, out _, out _);
